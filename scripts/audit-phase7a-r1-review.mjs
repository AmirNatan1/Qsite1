...
function validateMeasuredVisibility(geometry, visibility, label, expectedViewport, requireFullGeometry = false) {
  ...
  const viewport = rect(geometry.viewport, `${label} viewport`); const usable = rect(geometry.usableClipBounds, `${label} usable clip`); const effective = rect(geometry.effectiveVisibleBounds, `${label} effective visible`); const h1 = rect(geometry.h1?.rect, `${label} H1`); const glyphs = rect(geometry.glyphBounds, `${label} glyphs`);
  const header = geometry.occludingHeader;
  const headerRect = rect(header?.rect, `${label} sticky header`); const presentation = header?.presentation;
  ...
  const visibilityEffective = rect(visibility.effectiveVisibleBounds, `${label} visibility-summary effective visible`); const visibilityH1 = rect(visibility.h1Bounds, `${label} visibility-summary H1`); const visibilityGlyphs = rect(visibility.glyphBounds, `${label} visibility-summary glyphs`);
  ...
  invariant(allowances.every((value) => value >= MINIMUM_MANIFESTO_SAFETY_PX), `${label} intersects an effective clipping boundary`);
