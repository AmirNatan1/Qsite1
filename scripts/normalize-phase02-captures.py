"""Normalize Phase 0.2 in-app-browser screenshots and bind their lineage.
...
the exact requested CSS dimensions with Lanczos. The inner iframe always kept
its exact requested CSS layout. Both files and the complete scale/crop lineage
are bound into the browser matrix.
"""
...
PLAN_PATH = ROOT / "prototypes/phase-0-portal-layout-qa/capture-plan.json"
MATRIX_PATH = ROOT / "artifacts/evidence/phase-0-3d-repair-v2/browser-matrix-report.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="validate raw captures and report intended outputs without writing PNGs or the matrix",
    )
    ...
    if failures:
        for failure in failures:
            print(f"- {failure}")
        raise SystemExit(f"Phase 0.2 capture normalization stopped with {len(failures)} issue(s).")
    ...
    print(f"{action} {normalized_count} required Phase 0.2 browser captures.")
