...
const R1_REQUIRED_ROLE_INVENTORY = Object.freeze({
  ...
  "r1-native-windows-input-recording": Object.freeze({ section: "03-homepage-motion", kind: "video", minimum: 1, exact: 1 }),
  ...
  "r1-host-validation-summary": Object.freeze({ section: "00-provenance", kind: "document", minimum: 1, exact: 1 }),
  ...
  "r1-installed-chrome-zoom-recording": Object.freeze({ section: "09-accessibility", kind: "video", minimum: 1, exact: 1 }),
  "physical-device-result": Object.freeze({ section: "11-physical-device", kind: "document", minimum: 1, exact: 1 }),
  // Human recordings are optional under the superseding R1 authority. Their
  // absence is carried by the hash-bound ledger plus environmental report.
...
function validateR1HostValidation(entries, roleRecords) {
  const records = roleRecords.get("r1-host-validation-summary") ?? [];
  if (records.length !== 1 || records[0].path !== "00-provenance/host-validation-closure.json") throw new Error("R1 canonical host-validation summary inventory differs");
  const wrapper = r1DistilledWrapper(entries, records[0].path, "r1-host-validation-summary");
  ...
  if (wrapper.status !== "LIMITATION" || document?.schema !== R1_HOST_VALIDATION_SCHEMA || document.status !== "CAPTURED"
    ...
    || !Array.isArray(document.sourceReports) || stableJson(document.sourceReports.map(({ id }) => id)) !== stableJson(sourceIds)) throw new Error("R1 canonical host-validation wrapper differs");
  ...
  for (const report of document.sourceReports) {
    if (!report || !report.payload || typeof report.payload !== "object" || Array.isArray(report.payload) || reports.has(report.id)) {
      throw new Error(`R1 canonical host source binding differs: ${report?.id ?? "unknown"}`);
    }
    ...
    if (report.sha256 !== sha256(payloadBytes) || report.byteSize !== payloadBytes.length
      ...
      || !HASH64.test(report.source.sha256 ?? "") || !Number.isSafeInteger(report.source.byteSize) || report.source.byteSize <= 0) throw new Error(`R1 canonical host source binding differs: ${report?.id ?? "unknown"}`);
  ...
  const deploymentAuthority = parseR1Json(entries.get("00-provenance/deployment-authority-summary.json"), "00-provenance/deployment-authority-summary.json");
  ...
  if ((!directlyTestedFinalDeployment && !parityBoundFromDirectParent)
    || document.sourceReports.some(({ payload }) => typeof payload?.baseUrl === "string" && payload.baseUrl !== document.baseUrl)) throw new Error("R1 canonical host deployment binding differs");
  if (zoom?.status !== "PASS" || zoom.classification !== "GENUINE INSTALLED GOOGLE CHROME BROWSER ZOOM" || zoom.genuineInstalledChrome !== true || zoom.zoomPercent !== 200 || zoom.proxy !== false || zoom.sourceId !== "installed-chrome-200-percent"
    ...
    || zoomSource.results.some((result) => result.status !== "PASS" || !Array.isArray(result.failures) || result.failures.length)) throw new Error("R1 canonical installed-Chrome zoom authority differs");
  if (host.nativeWindowsInput?.status !== "PASS" || host.nativeWindowsInput.classification !== "NATIVE WINDOWS INPUT INJECTION" || host.nativeWindowsInput.physicalHumanMouse !== false || host.nativeWindowsInput.windowsSendInput !== true || host.nativeWindowsInput.sourceId !== "native-windows-input"
    || nativeSource?.schema !== "quantum-hub.phase-6-r1.native-windows-input.v1" || nativeSource.status !== "PASS" || nativeSource.method?.physicalHumanMouse !== false || nativeSource.method?.playwrightWheel !== false || nativeSource.method?.domWheelDispatch !== false || stableJson(Object.keys(nativeSource.checks ?? {}).sort(lexicalCompare)) !== stableJson([...nativeChecks].sort(lexicalCompare)) || Object.values(nativeSource.checks).some((value) => value !== true)) throw new Error("R1 canonical native Windows input authority differs");
  if (host.hiddenVisible?.status !== "NOT OBSERVED" || host.hiddenVisible.transitionObserved !== false || host.hiddenVisible.sourceId !== "real-hidden-visible" || hiddenSource?.schema !== "quantum-hub.phase-6-r1.real-chrome-hidden-visible.v1" || hiddenSource.status !== "NOT OBSERVED" || !Array.isArray(hiddenSource.attempts) || hiddenSource.attempts.length < 5 || hiddenSource.attempts.some((attempt) => attempt.hiddenObserved !== false || attempt.visibilityEvents?.length)) throw new Error("R1 canonical hidden/visible authority differs");
  if (host.bfcache?.status !== "NOT OBSERVED" || host.bfcache.persistedTrue !== 0 || host.bfcache.sourceId !== "bfcache-multiengine" || !Number.isSafeInteger(host.bfcache.trials) || host.bfcache.trials < 3 || bfcacheSource?.status !== "NOT OBSERVED" || bfcacheSource.persistedTrue !== 0 || bfcacheSource.trials !== host.bfcache.trials) throw new Error("R1 canonical BFCache host authority differs");
  ...
  for (const binding of mediaBindings) {
    ...
    if (!file || !recording || !bytes || file.bytes !== bytes.length || file.sha256 !== sha256(bytes) || recording.validation?.status !== "PASS" || Object.values(recording.validation?.checks ?? {}).some((value) => value !== true)) throw new Error(`R1 canonical host recording binding differs: ${binding.relativePath}`);
  }
  return document;
}
...
function validateR1NodeAndChangeAuthority(entries, roleRecords) {
  ...
  if (changeLedger.schema !== `${EVIDENCE_ASSEMBLY_SCHEMA}.change-ledger` || changeLedger.status !== "PASS"
    ...
    throw new Error("R1 canonical change ledger differs from the exact path authority");
  }
  const deployment = parseR1Json(entries.get(DEPLOYMENT_VERIFICATION_PATH), DEPLOYMENT_VERIFICATION_PATH);
...
export function validateR1CanonicalEvidencePayload(payloadEntries) {
  ...
  const humanLedgerWrapper = parseR1Json(entries.get(R1_HUMAN_LEDGER_PATH), R1_HUMAN_LEDGER_PATH);
  ...
  if (records.length !== expectedPathRoles.size) throw new Error("R1 canonical evidence inventory path/role topology differs");
  const recordPaths = [];
  ...
  const hostValidation = validateR1HostValidation(entries, roleRecords);
  ...
  const humanZoomStatuses = Array.isArray(zoom?.routeOutcomes) ? zoom.routeOutcomes.map(({ status }) => status) : [];
  const zoomStatus = r1Aggregate([hostValidation.hostValidation.chromeZoom.status, ...humanZoomStatuses], "PENDING HUMAN REVIEW");
  ...
  const humanHiddenVisible = unavailable ? null : lifecycleChecks.includes(false) ? "FAIL" : lifecycleChecks.every((value) => value === true) ? "PASS" : "PENDING HUMAN REVIEW";
  const hiddenVisible = r1Aggregate([...r1MachineLifecycleStatuses(entries, roleRecords, "visibility"), hostValidation.hostValidation.hiddenVisible.status, humanHiddenVisible].filter(Boolean), "NOT OBSERVED");
...
function validateR1HumanLedgerSemantics(ledger) {
  ...
  exactJson(ledger.policy, { filePresenceIsPass: false, machineRecordingSubstitutionAllowed: false, failRequiresTimestampOrFrame: true, allFourFilesRequiredBeforePackaging: false, unavailableHardwareDoesNotBlockPackaging: true }, "R1 human-evidence ledger policy");
...
function validateR1EnvironmentalLimitationsReport(text) {
...
export function validateR1HumanEvidencePayload(entries) {
  ...
  if (unavailable) {
    const limitations = byPath.get(R1_ENVIRONMENTAL_LIMITATIONS_PATH)?.toString("utf8");
    validateR1EnvironmentalLimitationsReport(limitations);
    return {
