...
if (cinematicChunk) {
  ...
  check((cinematicChunk.text.match(/\.play\(/g) ?? []).length === 0 && !/requestVideoFrameCallback|wake-forward|wake-reverse|wake-armed/.test(cinematicChunk.text) && /scroll-addressed/.test(cinematicChunk.text), "scroll-addressed-crt", `dist/${cinematicChunk.path}`, "production output must keep the one decoder paused and address the complete CRT startup from native document position");
