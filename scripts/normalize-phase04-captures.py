"""Normalize and verify Phase 0.4 repository-native browser captures.
...
PLAN_PATH = ROOT / "prototypes/phase-0-4-crt-portal-qa/capture-plan.json"
EVIDENCE_PATH = ROOT / "artifacts/evidence/phase-0-4-crt-television"
...
PLAN_SCHEMA = "quantum-hub.phase-0-4-crt-television.typography-capture-plan.v1"
MATRIX_SCHEMA = "quantum-hub.phase-0-4-crt-television.typography-collision-matrix.v1"
CHECKPOINT_SCHEMA = "quantum-hub.phase-0-4-crt-television.capture-checkpoint.v1"
...
def validate_authority(plan: dict[str, Any], matrix: dict[str, Any], *, check_only: bool) -> None:
    if plan.get("schema") != PLAN_SCHEMA:
        raise SystemExit(f"Unexpected Phase 0.4 plan schema: {plan.get('schema')}")
    freeze = plan.get("sceneFreeze", {})
    if freeze.get("status") != "frozen" or freeze.get("captureAllowed") is not True:
        raise SystemExit("Phase 0.4 normalization is locked until the CRT scene/keepout authority is frozen.")
    matrix_status = freeze.get("matrixStatus")
    if not check_only and matrix_status != "ready-for-capture":
        raise SystemExit("Phase 0.4 normalization writes are allowed only while matrixStatus is ready-for-capture.")
    if check_only and matrix_status not in {"ready-for-capture", "complete"}:
        raise SystemExit("Phase 0.4 normalization checks require a ready-for-capture or complete authority.")
    if matrix.get("schema") != MATRIX_SCHEMA:
        raise SystemExit(f"Unexpected Phase 0.4 matrix schema: {matrix.get('schema')}")
...
def main() -> None:
    ...
    if plan.get("capture", {}).get("normalizationMethod") != (
        ...
        raise SystemExit("Unexpected Phase 0.4 normalization method")
