...
check(/conceptualCoordinateForScroll/.test(cinematicController) && /arrivalScrollOffset/.test(cinematicController) && /physicalFrameFor/.test(cinematicController) && /FIRST_CHANGED_FRAME\s*=\s*46/.test(cinematicController) && /ARRIVAL_FRAME\s*=\s*285/.test(cinematicController) && /STABLE_Q_FRAME\s*=\s*370/.test(cinematicController), "deterministic-mapping", "src/scripts/home-cinematic-integration.ts", "integer scroll mapping must keep exact F1 at top, enter F46 on the first positive pixel, and reserve F285..F370 for one bounded reaction");
check(/video\.pause\(\)/.test(cinematicController) && /video\.currentTime\s*=\s*targetTime/.test(cinematicController), "paused-seek-surface", "src/scripts/home-cinematic-integration.ts", "scroll and reverse authority must use direct paused seeks outside the bounded automatic wake");
check(/video\.seeking/.test(cinematicController) && /addEventListener\(\s*["']seeked["'][\s\S]{0,700}requestCurrentFrame\(\)/.test(cinematicController), "latest-seek-coalescing", "src/scripts/home-cinematic-integration.ts", "an in-flight decoder seek must collapse subsequent updates to the newest pending frame");
check(matches(cinematicController, /video\.play\s*\(/g) === 1 && /requestVideoFrameCallback/.test(cinematicController) && /cancelVideoFrameCallback/.test(cinematicController) && /mediaTime\s*>=\s*STABLE_Q_TIME\s*-\s*0\.5\s*\/\s*FRAME_RATE/.test(cinematicController), "bounded-auto-wake", "src/scripts/home-cinematic-integration.ts", "the one decoder may play only the F285-to-F370 wake and must stop on presentation authority with a requestAnimationFrame fallback");
check(/transitionReaction/.test(cinematicController) && /wake-reverse/.test(cinematicController) && /crossedForward/.test(cinematicController) && /crossedReverse/.test(cinematicController), "reaction-state-machine", "src/scripts/home-cinematic-integration.ts", "forward override, reverse unwind, and re-entry must be explicit deterministic states driven by native scroll position");
...
check(matches(cinematicController, /\b(?:window\.)?setTimeout\s*\(/g) <= 1 && /decode-timeout|load-timeout/.test(cinematicController), "no-playback-timer", "src/scripts/home-cinematic-integration.ts", "the sole permitted timeout is a media fail-open watchdog; wake timing must use decoder presentation callbacks or animation frames");
