...
const port = Number.parseInt(portArgument?.slice(7) ?? process.env.PHASE5AR_ROUTE_PORT ?? "4176", 10);
const boardNames = new Set(["page", "signature", "materials"]);
...
const headers = {
  ...
  "x-phase5ar-local-prototype": "QH_PHASE5AR_ROUTE_LAB_ONLY",
};
...
const server = createServer((request, response) => {
  ...
  if (pathname === "/system/") html = renderSystem();
    ...
    response.writeHead(404, { ...headers, "content-type": "text/plain; charset=utf-8" }).end("Local Phase 5A-R prototype route not found");
    return;
  }
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.log(`Phase 5A-R supporting-route lab: http://127.0.0.1:${boundPort}/for-partners/`);
});
