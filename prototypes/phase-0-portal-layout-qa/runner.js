...
if (!allowedWidths.has(width) || !allowedHeights.has(height)) {
  throw new Error(`Unsupported Phase 0.3 viewport: ${width}x${height}`);
}
...
if (!Number.isFinite(captureScale) || Math.abs(captureScale - expectedCaptureScale) > 0.000001) {
  throw new Error(
    `Unsupported Phase 0.3 captureScale ${params.get("captureScale")}; expected ${expectedCaptureScale} for ${width}px`,
  );
}
...
const elements = {
  ...
  reportNode: document.querySelector("#phase03-runner-report"),
  legacyReportNode: document.querySelector("#phase02-runner-report"),
};
...
async function run() {
  ...
  await (child.phase03Ready ?? child.phase02Ready);
  const report = await (child.runPhase03TypographyCheck ?? child.runPhase02TypographyCheck)();
  ...
  const activeElement = child.document.activeElement;
  const focusedReviewControl =
    activeElement instanceof child.HTMLElement && activeElement.matches("#review-surface button")
      ? activeElement
      : null;
  const focusState = {
  ...
  const runnerReport = {
    schema: "quantum-hub.phase-0-3d-repair-v3.exact-viewport-runner-report.v1",
  ...
  runnerReport.pass =
    ...
    report.pass === true;
  window.phase03RunnerReport = runnerReport;
  window.phase02RunnerReport = runnerReport;
  elements.reportNode.textContent = JSON.stringify(runnerReport);
  elements.legacyReportNode.textContent = JSON.stringify(runnerReport);
  ...
  window.parent.postMessage({ type: "phase03-runner-report", report: runnerReport }, window.location.origin);
  return runnerReport;
}

const runnerReadiness = run().catch((error) => {
  elements.body.dataset.ready = "error";
  elements.status.textContent = error.message;
  throw error;
});
window.phase03RunnerReady = runnerReadiness;
window.phase02RunnerReady = runnerReadiness;
