...
A later isolated Chromium run exposed a separate sampling race in the same evidence harness: the correct skip link held focus and focus styling, but the fixed post-Tab delay captured its 140 ms entrance transition while its rectangle was still outside the viewport. A final busy-host replay also showed that request-animation-frame polling could exhaust an overly narrow 1.5 s bound immediately before the canonical observation became fully visible. The runner now polls deterministically every 50 ms, within a strict 5 s maximum, for the focused element's complete viewport geometry before recording the observation. A genuine timeout still produces the ordinary structured focus failure. This is a tooling-only timing repair and does not alter the accepted skip-link CSS or any production source.

The first post-commit Chromium replay then exposed a third tooling-only classification defect: supporting-route forward focus was required to land on anchors even when the actual sequential controls were Maradin's native buttons or SPARK's native summary disclosures. Both observations carried complete in-viewport geometry, the exact visible focus outline, rendered ancestor chains, and coherent reverse focus. The validator now binds the accepted route order exactly—buttons for Maradin, summaries for SPARK, and anchors for the other routes—retains the stricter exact audience-link requirement on Home, and continues to reject arbitrary or route-mismatched focus targets. No production source was changed.

A post-commit Firefox matrix then caught a Home readiness race: the native skip activation reached the exact visible `/#entry` target and the following Tab entered the correct audience trajectory, but the dynamic Home controller had not yet installed its focus handoff, leaving the sampled active ID empty. An independent headed Firefox replay after controller initialization retained `section#entry` focus through the trusted Enter/click, hashchange, animation frame, and 500 ms observation. The harness now waits explicitly for the enhanced Home controller, exact skip link, and focusable entry target before beginning the keyboard sequence; a timeout remains a structured failure. No production source was changed.

The isolated WebKit axe matrix exposed an engine event-ordering difference: its expected 404 console event retained the immediately previous same-origin page URL while the console location and main-frame request ledger both identified the exact intentional 404. The diagnostic validator now requires the exact same-origin 404 console location plus the exact 404 navigation request, while allowing that stale same-origin document URL; cross-origin or wrong-location errors remain failures. This repairs evidence attribution only and does not suppress unexpected console errors.
