"""Render deterministic Phase 3-R frames from the repaired derivative.

Raw frame sequences must be directed outside the repository. Desktop and
mobile select separate procedural raster authorities while sharing the exact
accepted physical scene, timeline, and camera compositions.
"""
...
optix_cache = Path(os.environ.get("TEMP", str(Path.home()))) / "QuantumPhase3ROptixCache"
...
def parse_args() -> dict:
    ...
    result = {
        "variant": "desktop",
        "quality": "preview",
        "frames": ",".join(str(frame) for frame in cfg.REPAIR_REVIEW_FRAMES.values()),
        "output": None,
        "samples": None,
    }
...
def assign_material(obj: bpy.types.Object, material_name: str) -> None:
    ...
    if material is None:
        raise RuntimeError(f"missing Phase 3-R variant material: {material_name}")
    obj.data.materials.clear()
    obj.data.materials.append(material)


def configure_screen_variant(variant: str) -> None:
    title = variant.title()
    assign_material(bpy.data.objects["CRT_InternalPhosphorLayer"], f"Phase3R_PhosphorField_{title}")
    interface = bpy.data.collections["CRT_PHYSICAL_SIGNAL_INTERFACE"]
    for obj in interface.all_objects:
        stage = str(obj.get("interface_stage", "none"))
        if stage in {"brand", "route", "ready"}:
            assign_material(obj, f"Phase3R_Interface_{stage.title()}_{title}")
...
def configure_variant(variant: str) -> None:
    ...
    configure_screen_variant(variant)
...
def main() -> None:
    if Path(bpy.data.filepath).resolve() != cfg.DERIVATIVE_SOURCE.resolve():
        raise RuntimeError("renderer must open the Phase 3-R derivative authority")
    args = parse_args()
    ...
    for order, frame in enumerate(frames, 1):
        ...
        path = output / f"phase3r-{args['variant']}-{frame:04d}.png"
        ...
        print(
            f"QH_PHASE3R_FRAME={frame} VARIANT={args['variant']} "
            f"ORDER={order}/{len(frames)} SECONDS={elapsed:.3f}"
        )
    report = {
        "schema": "quantum-hub.phase-3-r-crt-authenticity.raw-render-report.v1",
        "variant": args["variant"],
        "quality": args["quality"],
        "resolution": [scene.render.resolution_x, scene.render.resolution_y],
        "samples": scene.cycles.samples,
        "engine": scene.render.engine,
        "source": cfg.DERIVATIVE_SOURCE.name,
        "devices": devices,
        "frames": records,
        "total_seconds": round(time.perf_counter() - started, 4),
    }
    report_path = output / f"phase3r-{args['variant']}-{args['quality']}-render-report.json"
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(f"QH_PHASE3R_RENDER_REPORT={report_path}")
