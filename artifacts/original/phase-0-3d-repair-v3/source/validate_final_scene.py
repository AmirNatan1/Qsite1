"""Machine-validate the final Phase 0.3 Blender source contract.
...
def main() -> None:
    ...
    blade = bpy.data.objects.get("Station_AsymmetricOpticalBlade")
    spine = bpy.data.objects.get("Station_LoadBearingServiceSpine")
    glass = bpy.data.objects.get("Aperture_OpticallyBlackDormantGlass")
    foundation = bpy.data.objects.get("Station_BelowGradeIntegratedFoundation")
    interruptor = bpy.data.objects.get("Station_LowerRightStructuralInterruptor")
    blade_height = float(blade.dimensions.z) if blade else 0.0
    aperture_width = float(glass.dimensions.x) if glass else 0.0
    aperture_ratio = aperture_width / blade_height if blade_height else 0.0
    checks.append(
        check(
            "selected inclined-blade station preserves integrated aperture and installation anatomy",
            bool(
                blade
                and spine
                and glass
                and foundation
                and interruptor
                and 0.48 <= aperture_ratio <= 0.72
                and scene.get("selected_option") == "A / Inclined Optical Blade"
            ),
            {
                "selected_option": scene.get("selected_option"),
                "blade": blade.name if blade else None,
                "service_spine": spine.name if spine else None,
                "foundation": foundation.name if foundation else None,
                "structural_interruptor": interruptor.name if interruptor else None,
                "blade_height_m": round(blade_height, 6),
                "optical_glass_width_m": round(aperture_width, 6),
                "aperture_to_blade_height": round(aperture_ratio, 6),
                "nominal_station_dimensions_m": {"width": 1.86, "height": 1.24, "depth": 0.72},
            },
        )
    )
    ...
    mobile_sheath = bpy.data.objects.get("MobileCable_PhysicalGraphiteSheath")
    mobile_objects = [obj for obj in bpy.data.objects if obj.name.startswith("MobileCable_")]
    desktop_objects = [obj for obj in bpy.data.objects if obj.name.startswith("Cable_")]
    checks.append(
        check(
            "independent 2.25-turn portrait cable is exclusive by render composition",
            bool(
                mobile_sheath
                and abs(float(mobile_sheath.get("turns", 0.0)) - 2.25) < 1e-6
                and mobile_sheath.get("outer_to_inner") is True
                and all(obj.hide_render for obj in mobile_objects)
                and all(not obj.hide_render for obj in desktop_objects)
            ),
            {
                "object": mobile_sheath.name if mobile_sheath else None,
                "turns": float(mobile_sheath.get("turns", 0.0)) if mobile_sheath else None,
                "outer_to_inner": bool(mobile_sheath.get("outer_to_inner")) if mobile_sheath else None,
                "mobile_objects_hidden_by_default": all(obj.hide_render for obj in mobile_objects),
                "desktop_objects_visible_by_default": all(not obj.hide_render for obj in desktop_objects),
                "render_controller": "source/render_final_stills.py selects one family by camera",
            },
        )
    )
    ...
    dormant_materials = [
        "Cable_WarmMagenta_RecessedCore",
        "Cable_SingleFront_ModestHighlight",
        "Foundation_LocalArrival",
    ...
    mechanical = sorted(obj.name for obj in bpy.data.objects if obj.name.startswith("MechanicalResponse_"))
    mechanism_ids = sorted({str(bpy.data.objects[name].get("mechanism_id")) for name in mechanical})
    iris_material_strength = material_emission_strength("InternalIris_DarkMechanical")
    checks.append(
        check(
            "exactly one linked five-blade internal iris mechanism",
            len(mechanical) == 5 and mechanism_ids == ["internal-iris"] and iris_material_strength <= 0.07 and scene.get("mechanical_response_count") == 1,
            {
                "blade_count": len(mechanical),
                "objects": mechanical,
                "mechanism_ids": mechanism_ids,
                "scene_mechanism_count": scene.get("mechanical_response_count"),
                "response": "one linked five-blade internal iris",
                "dormant_iris_emission_strength": iris_material_strength,
            },
        )
    )
    ...
    checks.append(check("five still cameras span a 28-degree arc", len(arc_cameras) == 5 and abs(arc - 28.0) < 1e-6, {"cameras": arc_cameras, "span_degrees": arc}))
    ...
    checks.append(
        check(
            ...
            scene.render.engine == "BLENDER_EEVEE" and scene.eevee.taa_render_samples == 64,
            {
    ...
    manifest = {
        "schema": "quantum-hub.phase-0-3d-repair-v3.blender-source-validation.v1",
    ...
    print(f"QH_V3_VALIDATION_PASS={passed}")
    print(f"QH_V3_VALIDATION_CHECKS={len(checks)}")
    print(f"QH_V3_VALIDATION_MANIFEST={OUTPUT.resolve()}")
    if not passed:
        for item in checks:
            if not item["pass"]:
