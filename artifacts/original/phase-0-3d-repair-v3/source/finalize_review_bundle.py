"""Seal the exact thirteen-image Phase 0.3 human-review bundle."""
...
def main() -> None:
    ...
    for filename in REQUIRED:
        ...
        if filename == "aperture-station-silhouette-options.png":
            record = {
                **facts,
                "sourceManifest": "manifests/silhouette-decision-manifest.json",
                "source_paths": [item.get("source", item["path"]) for item in silhouette["source_renders"]],
    ...
    originals = {
        "schema": "quantum-hub.phase-0-3d-repair-v3.review-originals.v1",
        ...
        "final_blend": {
            "path": "source/quantum-aperture-station-v3.blend",
        },
        "portal_layout": {
    ...
    bundle = {
        "schema": "quantum-hub.phase-0-3d-repair-v3.review-bundle.v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "authorities": {
        ...
        "required_count": 13,
        "all_required_present": len(bundle_records) == 13,
    }
    bundle_path = MANIFESTS / "review-bundle-manifest.json"
    bundle_path.write_text(
        json.dumps(bundle, indent=2, ensure_ascii=True) + "\n", encoding="utf-8"
    )
    print("QH_V3_REVIEW_BUNDLE=PASS")
    print(f"QH_V3_REVIEW_RECORDS={len(bundle_records)}")
