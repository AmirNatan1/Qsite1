...
Status: locked production authoring contract
...
| Spiral conduction | 31–112 | `[0.111111, 0.414815)` | `[1.000, 3.733)` s | Current front has traversed the spiral and reached the physical rear connection |
| Arrival / indicator | 113–120 | `[0.414815, 0.444444)` | `[3.733, 4.000)` s | Local connector response occurs first, then the small physical indicator is awake; screen remains dark until the line begins |
...
| Settling / scanlines / black stabilization | 155–176 | `[0.570370, 0.651852)` | `[5.133, 5.867)` s | Restrained degauss response is settled, fine scanlines are present, and raster black is stable |
...
| Camera approach / portal handoff | 211–270 | `[0.777778, 1.000000]` | `[7.000, 9.000]` s | Cabinet is effectively outside the viewport; raster is frontal, nearly flat, text-safe, and visually aligned for Phase 2B ENTRY |
...
- Frame 112 is the last conduction frame. It may show contact, but the localized connector reaction belongs to frame 113.
...
### Frames 113–120 — arrival and indicator

- Frames 113–115 contain a small localized electrical response at the rear connection and strain relief.
...
- Frames 155–160 provide the restrained degauss/geometry-breathing response.
- Frames 161–166 reduce instability without a discontinuity.
- Frames 167–171 introduce fine, physically plausible scanlines.
- Frames 172–176 stabilize black level and phosphor response.
...
- Frames 177–186 establish `QUANTUM HUB`.
- Frames 187–198 add `FRAME   SOURCE   ASSESS   TEST   DECIDE` as one restrained route line.
- Frames 199–210 resolve the single status `TEST ROUTE AVAILABLE`, preserving the accepted CRT content authority.
...
### Frames 211–270 — camera approach and handoff

- Frames 211–228 begin the meaningful physical approach while retaining cabinet, glass, and proving-field context.
- Frames 229–246 progressively correct toward frontal alignment. Perspective correction must already be obvious before the late portal phase.
- Frames 247–258 begin late flattening: the glass dominates, curvature and phosphor softness diminish perceptually, and scanlines become fainter without disappearing abruptly.
- Frames 259–264 take the screen edges beyond the viewport and make the cabinet/bezel effectively absent. Raster black converges on the accepted Operating Field black.
- Frames 265–269 lock the Phase 2B ENTRY alignment, retire physical-screen text before it could duplicate native semantic text, and retain visible portal-field logic rather than a blank bridge.
...
| Current reaches connection | 112 | `0.412639` | `00:03.700` |
| Connector response | 113 | `0.416357` | `00:03.733333` |
...
| Scanlines begin | 167 | `0.617100` | `00:05.533333` |
| Quantum brand begins | 177 | `0.654275` | `00:05.866667` |
| Route begins | 187 | `0.691450` | `00:06.200` |
| Status begins | 199 | `0.736059` | `00:06.600` |
| Camera approach begins | 211 | `0.780669` | `00:07.000` |
| Front-alignment phase begins | 229 | `0.847584` | `00:07.600` |
| Late flattening begins | 247 | `0.914498` | `00:08.200` |
| Raster becomes viewport-dominant | 259 | `0.959108` | `00:08.600` |
...
- Frame 270 back through 211 restores curvature, scanlines, glass, bezel, cabinet, oblique perspective, and proving-field context gradually.
- Frames 210 back through 177 withdraw status, route, and brand cleanly without popping or reordering.
